...
class GdbManager:
    ...
    def checkBinaryPath(self):
        if self.binaryPath == None:
            print "You should set binary path that contains tizenrt,\
            System.map files"
            return False
    ...
    def checkELF(self):
        if os.access("%s/tizenrt" % self.binaryPath, os.F_OK):
            return True
        print "ELF file(tizenrt) doesn't exist"
        return False
    ...
    def generateGdbCommands_artik051(self):
        ...
        gdbCmdFile.write("file %s\n"
                % (os.path.join(self.binaryPath, "tizenrt")))
    ...
    def runGdbCommands(self):
        out = os.system("arm-none-eabi-gdb %s --command=%s\n" \
                % (os.path.join(self.openocdPath, "binary/tizenrt"), \
...
def main():
    ...
    parser.add_option('-b', '--binary', dest='binaryPath',
            ...
            metavar='PATH',
            help="Specify tizenrt binary path, "
            "[default:%default]")
