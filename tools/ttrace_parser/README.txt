...
2. Dump from artik053 device
  ...
  1. HOST$ ./ttrace_tizenrt.py -d artik053
  2. artik053$ ttrace -s apps libs ipc lock task
  3. artik053$ ttrace -f
  4. artik053$ ttrace -d

  Dump feature needs tizenrt binary, System.map, artik053.cfg and openocd executable,
  They are located in tizenrt/build/output/bin and
  tizenrt/build/configs/artik053/tools/openocd.
  These paths are set by default.
  However, You had changed location of them,
  You can specified paths to ./scripts/ttrace_tizenrtDump.py with -b, -d options.

  for examples,
  $ HOST$ ./scripts/ttrace_tizenrtDump.py -t artik053 -b <binaryPath> -d <openocdPath>

Example
=======

  $ ./ttrace_tizenrt.py -i sample/sample_log
